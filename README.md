...
<div class="app">
  <header>
    ...
    <div class="topline">
      <div class="hint"Розробник - СІГСПП молодший сержант Войтович Євген • Інтерфейс українською • Зберігає історію в пам'яті браузера</div>
    </div>
  </header>
...
const RAW_JSON_TEXT = `{
  ...
  "questions": [
    ...
    [cite_start]"comment": "Картка вогню (Fire Card) — це ключовий тактичний документ, який командир відділення складає для організації оборони. На неї обов'язково наносяться: 1. **Орієнтири**, їх номери, найменування та відстані до них (не менше трьох)[cite: 211]. 2. [cite_start]**Положення противника** (не менше двох виявлених об'єктів)[cite: 212]. 3. [cite_start]**Позиція відділення**[cite: 213]. 4. [cite_start]**Основні та додаткові смуги вогню**[cite: 214]. 5. [cite_start]**Основна та запасна позиція для БМП**[cite: 215]. Також вказуються додаткові сектори обстрілу, зона зосередженого вогню та сектори ведення вогню для кожного бійця."
  },
  
  // Практичне завдання №2: Домедична допомога (MARCH)
  {
    "id": 102,
    "category": "Практичні завдання (Розрахунки та Алгоритми)",
    "difficulty": 3,
    ...
    "comment": "Алгоритм дій в зоні обстрілу (Care Under Fire - CUF) є першим етапом TCCC (Tactical Combat Casualty Care). [cite_start]Пріоритет: **ВОГОНЬ У ВІДПОВІДЬ І УКРИТТЯ!**[cite: 106]. 1. Придушити вогонь противника. 2. Визначити, чи поранений може переміститися в укриття самостійно. 3. [cite_start]Якщо поранений має масивну кровотечу, що загрожує життю, **накласти турнікет** (CAT) на високе положення над раною (самодопомога або допомога товариша)[cite: 108]. **НЕ ПРОВОДИТИ** інші маніпуляції (перевірка дихальних шляхів, оцінка шоку) до переміщення в зону укриття."
  },
  
  // Практичне завдання №3: Топографія (відстань за масштабом)
  {
    "id": 103,
    "category": "Практичні завдання (Розрахунки та Алгоритми)",
    "difficulty": 2,
    ...
    "comment": "Визначення відстані за чисельним масштабом: **Р (км) = Р (см) × М (км)**. [cite_start]Де: 1. **Р (см)** — вимірювальна відстань між точками на карті (в сантиметрах)[cite: 221]. 2. **М (км)** — величина масштабу, що показує, скільки кілометрів на місцевості відповідає 1 см на карті (наприклад, для масштабу 1:50000, 1 см = 0.5 км). [cite_start]Наприклад: на карті 1:50000 відстань 4 см → $4 \text{ см} \times 0.5 \text{ км/см} = 2 \text{ км}$[cite: 221]."
  },

  // Практичне завдання №4: Топографія (знайти за координатами)
  {
    "id": 104,
    ...
    "comment": "Алгоритм пошуку об'єкта за прямокутними координатами (наприклад, 48 35700 м - 34 82100 м): 1. **Визначити квадрат:** Знайти на карті вертикальну лінію, що відповідає першим двом цифрам Сходу (X) (наприклад, 82) та горизонтальну лінію, що відповідає першим двом цифрам Півночі (Y) (наприклад, 35). 2. **Розділити квадрат:** Використовуючи третю (сотні метрів) та четверту (десятки метрів) цифри, відкласти на схід від початкової вертикальної лінії (наприклад, 100 м) та на північ від початкової горизонтальної лінії (наприклад, 700 м). [cite_start]Це і буде місцезнаходження об'єкта[cite: 224]."
  },

  // Практичне завдання №5: Топографія (визначити координати)
  {
    "id": 105,
    "category": "Практичні завдання (Топографія)",
    "difficulty": 2,
  ...
  // Практичне завдання №6: Вогнева підготовка (поправка на температуру)
  {
    ...
    "comment": "1. **Визначення прицілу:** Для АК-74 (пристріляного на позначку 'П') на відстані 450 м ціль уражається прицілом «4» або «5». 2. **Поправка на мороз:** При температурі повітря **нижче 0°C** (в даному випадку -26°C), траєкторія кулі стає пологішою через зміну порохового заряду. При температурі **нижче -25°C** (або -20°C, залежно від інструкції) на відстані **до 500 м** приціл збільшується на **один розподіл**. [cite_start]Отже, замість «4» використовуємо **«5»**[cite: 232]. 3. **Додаткова поправка:** Оскільки ціль ростова і стоїть на відстані 450 м (що є між 400 і 500 м), стрілець цілиться під зріз цілі (в її пояс/нижню частину), щоб уразити її в груди."
  },

  // Практичне завдання №7: Вогнева підготовка (розрахунок патронів)
  {
    "id": 107,
    "category": "Практичні завдання (Розрахунки та Алгоритми)",
    "difficulty": 3,
    ...
    "comment": "Середня кількість патронів (n) для ураження цілі однією чергою визначається за формулою: $n = s / P1$. Де: **s** — кількість патронів у черзі (3 патрони). [cite_start]**P1** — вірогідність ураження цілі однією чергою (0.54)[cite: 235]. **Розрахунок:** $n = 3 / 0.54 \approx 5.56$ патронів. **Висновок:** Оскільки не можна витратити частину патрона, середній розрахунок округлюється до **6 патронів**. [cite_start]Тобто, для вирішення вогневого завдання стрілець повинен зробити **дві прицільних черги по три патрони**[cite: 235, 236]."
  },

  // Практичне завдання №8: Вогнева підготовка (дальність по спалаху)
  {
    "id": 108,
    "category": "Практичні завдання (Розрахунки та Алгоритми)",
    "difficulty": 1,
    ...
    "comment": "Визначення відстані по звуку базується на швидкості поширення звуку в повітрі. [cite_start]Швидкість звуку приймається приблизно $340-350 \text{ м/с}$[cite: 239]. **Формула:** $Відстань (\text{м}) = Швидкість звуку (\text{м/с}) \times Час (\text{с})$. [cite_start]**Розрахунок:** $350 \text{ м/с} \times 2 \text{ с} = 700 \text{ метрів}$[cite: 239]. **Примітка:** Світло від спалаху доходить миттєво, тому час між спалахом і звуком є чистим часом проходження звуку."
  },

  // Практичне завдання №9: Вогнева підготовка (упередження по рухомій цілі)
  {
    "id": 109,
    "category": "Практичні завдання (Вогнева підготовка)",
    "difficulty": 3,
    "question": "Практичне завдання №9: Стрілець противника рухається на відстані 250 м (по флангу), швидкість 3 м/с (10 км/год). Що необхідно зробити, щоб вразити ціль (АКМ/АК-74)?",
    "answers": [
      {
    ...
    "comment": "Для ураження рухомої цілі необхідно взяти поправку (упередження). На дальності 250 м і швидкості $3 \text{ м/с}$, табличне упередження для стрільця, що біжить, становить приблизно **2-3 фігури** (корпуси). 1. [cite_start]**Упередження:** Взяти упередження приблизно **2.5 фігури** (відповідно до табличних даних)[cite: 243]. 2. [cite_start]**Приціл:** На відстані 250 м встановлюється приціл **«3»** (для АКМ)[cite: 244]. 3. **Точка прицілювання:** Прицілюватися слід **під зріз цілі** (в ноги або нижню частину корпуса), щоб вразити ціль в пояс/груди. Це компенсація вертикального падіння кулі."
  },

  // Практичне завдання №10: Вогнева підготовка (стрільба зверху-вниз РПК)
  {
    "id": 110,
    "category": "Практичні завдання (Вогнева підготовка)",
    "difficulty": 3,
    "question": "Практичне завдання №10: Кулеметник з РПК веде вогонь зверху-вниз на відстань 700 м під кутом приблизно $40^\circ$. На яку позначку потрібно встановити приціл?",
    "answers": [
      {
    ...
    "comment": "При стрільбі **зверху-вниз** (з гори, високої будівлі), траєкторія кулі є більш **пологою** відносно цілі, ніж при стрільбі з горизонтальної поверхні. Для ураження цілі на відстані $700 \text{ м}$ з РПК (приціл «7») необхідно: **Зменшити приціл** на один-два розподіли. Це компенсує балістичний ефект, коли куля фактично летить меншу відстань по горизонталі до цілі. [cite_start]Відповідно до інструкцій, на 700 м приціл зменшується до **«6»**[cite: 247]."
  },

  // Практичне завдання №11: Вогнева підготовка (упередження при боковому вітрі)
  {
    "id": 111,
    "category": "Практичні завдання (Вогнева підготовка)",
    "difficulty": 3,
    "question": "Практичне завдання №11: На відстані 400 м від стрілка знаходиться ростова фігура. Боковий вітер $12 \text{ м/с}$. Яке потрібно взяти упередження при стрільбі з АКМ?",
    "answers": [
      {
    ...
    "comment": "Для компенсації бічного вітру необхідно взяти поправку (упередження) у бік вітру. 1. [cite_start]**Базове упередження:** Відповідно до таблиць, при швидкості вітру $4 \text{ м/с}$ на відстані $400 \text{ м}$ упередження для ростової фігури становить приблизно **1.5 фігури**[cite: 251]. 2. **Розрахунок:** Вітер $12 \text{ м/с}$ в 3 рази сильніший, ніж базовий ($12 / 4 = 3$). 3. **Кінцеве упередження:** $1.5 \text{ фігури} \times 3 = **4.5 \text{ фігури}$**. [cite_start]**Висновок:** Стрілець повинен цілитися на **4.5 фігури** у бік, **звідки дме вітер**[cite: 251]. Наприклад, якщо вітер дме зліва, цілитися потрібно правіше цілі (на 4.5 фігури)."
  }
  ]
}
